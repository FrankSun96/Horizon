...
public partial class videoHome : System.Web.UI.Page
{
    #region Members
    ...
    const string COUNT_SQL = "select count(*)  from Video";
    #endregion
    ...
    public void Slider_Init()
    {
        ...
        string aSelectTop5GameImgQuery = "SELECT TOP 5 CoverImg, VideoID FROM Video ORDER BY Count DESC";
    ...
    {
        Response.Redirect("videos.aspx?videoID=" + id[0]);
    }
    protected void Image2_Click(object sender, ImageClickEventArgs e)
    {
        Response.Redirect("videos.aspx?videoID=" + id[1]);
    }
    protected void Image3_Click(object sender, ImageClickEventArgs e)
    {
        Response.Redirect("videos.aspx?videoID=" + id[2]);
    }
    protected void Image4_Click(object sender, ImageClickEventArgs e)
    {
        Response.Redirect("videos.aspx?videoID=" + id[3]);
    }
    protected void Image5_Click(object sender, ImageClickEventArgs e)
    {
        Response.Redirect("videos.aspx?videoID=" + id[4]);
    }
    ...
    private void GridViewDataBind(string sqlSearch)
    {
        ...
        {
            sqlResult = "Select Top " + PAGESIZE + "VideoID,VideoName,Date,CoverImg from Video where VideoID not in ( select top " + PAGESIZE * CurrentPage + " VideoID from Video order by " + ViewState["PageOrderBy"].ToString() + " DESC) order by " + ViewState["PageOrderBy"].ToString() + " DESC";
        }
        ...
        if (btn != null)
        {
            ...
            switch (cmd)//根据不同的CommandName做出不同的处理 
            {
                ...
                case "search":
                    ...
                    {
                        RecordsCount = GetRecordsCount("select count(*) from Video where VideoName like '" + SearchName.Text + "%'");//获取过滤后的总记录数  
                        ...
                        if (RecordsCount <= 10)
                            GotoPage.Enabled = false;
                        sqlResult = "Select Top " + PAGESIZE + "VideoID,VideoName,Date,CoverImg from Video where VideoID not in(select top " + PAGESIZE * CurrentPage + " VideoID from Video order by VideoID asc) and VideoName like '%" + SearchName.Text + "%' order by VideoID asc";
                    }
                    //                  ResetSearch();
                    break;
                case "jump":
    ...
    protected void orderBy_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        RecordsCount = GetRecordsCount("select count(*) from Video");//获取过滤后的总记录数  
        PagesCount = RecordsCount / PAGESIZE + OverPage();//该变量为页总数  
        ViewState["PagesCount"] = RecordsCount / PAGESIZE - ModPage();//该变量为末页索引，比页总数小1  
        ViewState["PageIndex"] = 0;//保存一个为0的页面索引值到ViewState，页面索引从0开始  
        ViewState["JumpPages"] = PagesCount;
        ...
        if (RecordsCount <= 10)
            GotoPage.Enabled = false;
        string   sqlResult = "Select Top " + PAGESIZE + "VideoID,CoverImg,Date,VideoNam from Video AND VideoID not in(select top " + PAGESIZE * CurrentPage + " VideoID from Video order by " + selected + " DESC) order by " + selected + " DESC";
        ViewState["PageIndex"] = CurrentPage;
        //将运算后的CurrentPage变量再次保存至ViewState 
        GridViewDataBind(sqlResult);//调用数据绑定函数TDataBind() 
    }
